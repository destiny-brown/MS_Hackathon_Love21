...
export default function DonorPortalPage() {
  ...
  useEffect(() => {
    ...
    api.recurringDonation()
      ...
      .catch((err) => setDonationStatus(err instanceof Error ? err.message : t("portal.donorLoadError")));
  }, [user, t]);

  if (loading || !user || user.role !== "donor") {
    return (
      <main className="flex min-h-screen items-center justify-center px-4 py-10">
        <p className="rounded-md border p-4 text-sm text-muted-foreground" role="status">{t("portal.checkingDonor")}</p>
      </main>
    );
  }
    ...
    <main className="min-h-screen px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl space-y-6">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">{t("portal.donorTitle")}</h1>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("portal.donorRecurringTitle")}</CardTitle>
            <CardDescription>{t("portal.donorRecurringDesc")}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-muted-foreground">
            <p>{t("portal.donorEndpoint")} {donationStatus || t("portal.loading")}</p>
            <Button type="button">{t("portal.donorManageBtn")}</Button>
          </CardContent>
        </Card>
      </div>
    </main>
