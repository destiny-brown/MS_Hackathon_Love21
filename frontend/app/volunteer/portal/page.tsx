...
export default function VolunteerPortalPage() {
  ...
  useEffect(() => {
    ...
    api.recurringDonation()
      ...
      .catch((err) => setDonationStatus(err instanceof Error ? err.message : "Could not load involvement tools"));
  }, [user]);

  if (loading || !user || user.role !== "volunteer") {
    return (
      <main className="flex min-h-screen items-center justify-center px-4 py-10">
        <p className="rounded-md border p-4 text-sm text-muted-foreground" role="status">Checking volunteer access...</p>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Track your involvement</CardTitle>
            <CardDescription>
              Initial volunteer sign-up stays public. This signed-in portal is for volunteers who opt in to track ongoing involvement.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-muted-foreground">
            <p>Backend involvement endpoint: {donationStatus || "Loading..."}</p>
            <Button type="button">Update volunteer availability</Button>
          </CardContent>
        </Card>
      </div>
    </main>
