...
export function SiteFooter() {
  ...
  const exploreLinks = footerNav.filter((item) =>
    ["/join-us", "/our-finance", "/members"].includes(item.href),
  );
  const moreLinks = footerNav.filter((item) =>
    ["/donate", "/our-volunteer", "/newsletter", "/contact-us"].includes(item.href),
  );
    ...
    <footer className="border-t border-brand-light bg-brand-light px-4 py-12 pb-28 sm:px-6 lg:px-8 lg:pb-12">
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-10 lg:grid-cols-3">
        <div className="col-span-2 lg:col-span-1">
          ...
          <p className="mt-3 text-sm text-brand-dark/70">{t("footer.tagline")}</p>
        </div>
        <div>
